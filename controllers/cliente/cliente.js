...
const clienteSocket = async (socket) => {
...
const addCliente = async (req, res) => {
    const { nombre, telefono, correo, placa_chasis } = req.body;
    const fecha_registro = new Date();

    if( !nombre, !telefono, !placa_chasis ) {
        return res.status(400).json({ error: "Nombre, teléfono y placa son obligatorios"});
    };
        ...
        db.query(checkQuery, [nombre, telefono], (error, results) => {
            ...
            if(results.length > 0){
                ...
                if(coincidenciaTelefono) {
                    return res.status(200).json({
                        error: "Ya existe un cliente con ese teléfono pero con diferencte nombre",
                        id_cliente: coincidenciaTelefono.id_cliente
                    });
                }
            }

            const insertQuery = 'INSERT INTO cliente (nombre, telefono, correo, placa_chasis, fecha_registro) VALUES (?, ?, ?, ?, ?)'
            db.query(insertQuery, [nombre, telefono, correo, placa_chasis, fecha_registro], (insertError, results) => {
                if(insertError) {
